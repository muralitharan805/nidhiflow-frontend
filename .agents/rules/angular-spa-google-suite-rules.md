...
- Angular apps built with `@angular/build:application` MUST specify `pages_build_output_dir = "dist/<project-name>/browser"` in `wrangler.jsonc` to ensure static `sitemap.xml` and `robots.txt` files residing in `public/` are served at root domain without SPA router 404 redirects.
